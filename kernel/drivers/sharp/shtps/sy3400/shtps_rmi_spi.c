...
struct shtps_rmi_spi {
	...
	#if defined(SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE)
		int vbus_en_pin;
	#endif /* SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE */
};
	...
	for(i = 0;i < shtps_get_fingermax(ts);i++){
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER /* info->fingers[i].state == SHTPS_TOUCH_STATE_PEN */){
			if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH){
				...
				last_history		= ts->drag_hist[i][xy].history[SHTPS_DRAG_SMOOTH_COUNT-1];
				last_history_FIXED	= SHTPS_DRAG_SMOOTH_INT_TO_FIXED(last_history);

				if(ts->drag_hist[i][xy].count == SHTPS_DRAG_SMOOTH_COUNT){
					inc_ave_FIXED = SHTPS_DRAG_SMOOTH_INT_TO_FIXED(last_history- ts->drag_hist[i][xy].history[0]) / (SHTPS_DRAG_SMOOTH_COUNT-1);
					if(xy == SHTPS_POSTYPE_X){
...
inline static void shtps_report_touch_pen_on(struct shtps_rmi_spi *ts, int pen, int x, int y, int w, int wx, int wy, int z)
{
	...
	#if defined ( SHTPS_TOUCH_EVENTLOG_ENABLE )
		if(shtps_touch_eventlog_is_recording() != 0){
			shtps_touch_eventlog_rec_event_info(SHTPS_TOUCH_STATE_PEN, finger, x, y, w, wx, wy, z);
		}
	#endif /* #if defined ( SHTPS_TOUCH_EVENTLOG_ENABLE ) */
}
...
static int __devinit shtps_rmi_probe(struct spi_device *spi)
{
	...
	ts->spi			= spi;
		...
		#if defined(SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE)
			ts->vbus_en_pin = of_get_named_gpio(spi->dev.of_node, "shtps_rmi,vbus_1_8V_en", 0);
		#endif /* SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE */
	#else
	...
	#if defined(SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE)
		shtps_vbus_ldo_enable(ts->vbus_en_pin, 1);
	#endif /* SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE */
...
static int __devexit shtps_rmi_remove(struct spi_device *spi)
{
	...
	if(ts){
		...
		#if defined(SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE)
			shtps_vbus_ldo_enable(ts->vbus_en_pin, 0);
		#endif /* SHTPS_1_8V_VBUS_LDO_CONTROL_ENABLE */
