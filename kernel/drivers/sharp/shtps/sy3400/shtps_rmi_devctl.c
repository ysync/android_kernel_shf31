...
void shtps_vbus_ldo_enable(int pin, u8 enable)
{
	int ret;
	
	if (gpio_is_valid(pin)) {
		ret = gpio_request(pin, "shtps_vbus_en_gpio");
		if (ret) {
			printk(KERN_ERR "[shtps] %s() : unable to request gpio [%d]\n", __func__, pin);
			goto err_en_gpio;
		}
		ret = gpio_direction_output(pin, enable);
		if (ret) {
			printk(KERN_ERR "[shtps] %s() : unable to set direction/value for gpio [%d]\n", __func__, pin);
			goto err_en_gpio;
		}
	} else {
		printk(KERN_ERR "[shtps] %s() : vbus en gpio not provided\n",  __func__);
	}
	
err_en_gpio:
	gpio_free(pin);
}
EXPORT_SYMBOL(shtps_vbus_ldo_enable);
