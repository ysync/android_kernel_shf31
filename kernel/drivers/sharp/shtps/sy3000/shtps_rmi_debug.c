 ...
 */
...
#if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 )
	#define F11_SET_DATA_FINGERSTATE(F11_2D_DATA01, id, para)	(F11_2D_DATA01[id / 4] = (F11_2D_DATA01[id / 4] & ~(0x03 << (2 * (id % 4)))) | ((para & 0x03) << (2 * (id % 4))))

	#define F11_SET_DATA_XPOS(F11_2D_DATA01, para)				{F11_2D_DATA01[0] = para >> 4 & 0xff; F11_2D_DATA01[2] = (F11_2D_DATA01[2] & 0xf0) | (para & 0x0f);}
	#define F11_SET_DATA_YPOS(F11_2D_DATA01, para)				{F11_2D_DATA01[1] = para >> 4 & 0xff; F11_2D_DATA01[2] = (F11_2D_DATA01[2] & 0x0f) | ((para & 0x0f) << 4);}
	#define F11_SET_DATA_WX(F11_2D_DATA01, para)				(F11_2D_DATA01[3] = (F11_2D_DATA01[3] & 0xf0) | (para & 0x0f))
	#define F11_SET_DATA_WY(F11_2D_DATA01, para)				(F11_2D_DATA01[3] = (F11_2D_DATA01[3] & 0x0f) | ((para & 0x0f) << 4))
	#define F11_SET_DATA_Z(F11_2D_DATA01, para)					(F11_2D_DATA01[4] = para)
	#define F11_SET_DATA_WX_H(F11_2D_DATA35, para)				(F11_2D_DATA35[0] = (F11_2D_DATA35[0] & 0xf0) | ((para & 0xf0) >> 4))
	#define F11_SET_DATA_WY_H(F11_2D_DATA35, para)				(F11_2D_DATA35[0] = (F11_2D_DATA35[0] & 0x0f) | (para & 0xf0))
#else
...
#endif /* #if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 ) */
	...
	#define	SHTPS_TOUCH_EVENTLOG_LOG_V(...)							\
		if(SHTPS_TOUCH_EVENTLOG_LOG_ENABLE != 0){					\
			printk(KERN_DEBUG "[shtps][eventlog] " __VA_ARGS__);	\
		}
	...
	#define SHTPS_TOUCH_EVENTLOG_LOG_FUNC_CALL()											\
		if(((shtps_get_logflag() & 0x02) != 0) || (SHTPS_TOUCH_EVENTLOG_LOG_ENABLE != 0)){	\
			printk(KERN_DEBUG "[shtps][eventlog] %s()\n", __func__);						\
		}
	#define SHTPS_TOUCH_EVENTLOG_LOG_FUNC_CALL_INPARAM(param)								\
		if(((shtps_get_logflag() & 0x02) != 0) || (SHTPS_TOUCH_EVENTLOG_LOG_ENABLE != 0)){	\
			printk(KERN_DEBUG "[shtps][eventlog] %s(%d)\n", __func__, param);				\
		}
...
#if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 )
	u8 	fingerMax = shtps_get_fingermax_from_outside();
	int	has_8bitW = shtps_has_8bitW_from_outside();
	int				F11_2D_DATA00_SIZE = 0;
#endif /* #if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 ) */
	
	SHTPS_TOUCH_EMU_LOG_V("shtps_touch_emu_set_finger_info() start\n");
#if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 )
	if(fingerMax > 8){
		F11_2D_DATA00_SIZE = 3;
	}else if (fingerMax > 4){
		F11_2D_DATA00_SIZE = 2;
	}else{
		F11_2D_DATA00_SIZE = 1;
	}
	buf[0] = 0;
	buf[1] = 0;

	for(i = 0; i < fingerMax; i++){
		u8 *F11_2D_DATA00 = buf + 2;
		u8 *F11_2D_DATA01 = buf + 2 + F11_2D_DATA00_SIZE + (5 * i);

		fw_repot_finger_tmp = &gShtps_touch_emu_info.fw_report_info.fingers[i];

		F11_SET_DATA_FINGERSTATE(	F11_2D_DATA00, i, fw_repot_finger_tmp->state);

		F11_SET_DATA_XPOS(			F11_2D_DATA01, fw_repot_finger_tmp->x);
		F11_SET_DATA_YPOS(			F11_2D_DATA01, fw_repot_finger_tmp->y);
		F11_SET_DATA_WX(			F11_2D_DATA01, fw_repot_finger_tmp->wx);
		F11_SET_DATA_WY(			F11_2D_DATA01, fw_repot_finger_tmp->wy);
		F11_SET_DATA_Z(				F11_2D_DATA01, fw_repot_finger_tmp->z);

		if(has_8bitW != 0){
			u8 *F11_2D_DATA35 = buf + 2 + F11_2D_DATA00_SIZE + (5 * fingerMax) + 1 + i;
			F11_SET_DATA_WX_H(		F11_2D_DATA35, fw_repot_finger_tmp->wx);
			F11_SET_DATA_WY_H(		F11_2D_DATA35, fw_repot_finger_tmp->wy);
		}
	}
#else
...
#if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 )
				u8 *F11_2D_DATA00 = buf + 2;
				u8 *F11_2D_DATA01 = buf + 2 + F11_2D_DATA00_SIZE + (5 * touch_list->finger_id);

				F11_SET_DATA_FINGERSTATE(	F11_2D_DATA00, touch_list->finger_id, touch_list->finger_state);

				F11_SET_DATA_XPOS(			F11_2D_DATA01, touch_list->finger_x);
				F11_SET_DATA_YPOS(			F11_2D_DATA01, touch_list->finger_y);
				F11_SET_DATA_WX(			F11_2D_DATA01, touch_list->finger_wx);
				F11_SET_DATA_WY(			F11_2D_DATA01, touch_list->finger_wy);
				F11_SET_DATA_Z(				F11_2D_DATA01, touch_list->finger_z);

				if(has_8bitW != 0){
					u8 *F11_2D_DATA35 = buf + 2 + F11_2D_DATA00_SIZE + (5 * fingerMax) + 1 + touch_list->finger_id;
					F11_SET_DATA_WX_H(		F11_2D_DATA35, touch_list->finger_wx);
					F11_SET_DATA_WY_H(		F11_2D_DATA35, touch_list->finger_wy);
				}
#else
...
int shtps_touch_emu_rec_finger_info(u8 *buf)
{
	...
	fingerNum = fingerMax;
	if(fingerNum > SHTPS_FINGER_MAX){
		fingerNum = SHTPS_FINGER_MAX;
	}
#endif /* #if defined ( SHTPS_TOUCH_EMURATOR_FOR_SY3200 ) */
	...
	for(i = 0; i < fingerNum; i++){
		...
		fw_report_info_tmp.fingers[i].state	= (buf[i / 4] >> (2 * (i % 4))) & 0x03;
		fw_report_info_tmp.fingers[i].x		= F11_DATA_XPOS(fingerInfo);
		fw_report_info_tmp.fingers[i].y		= F11_DATA_YPOS(fingerInfo);
		fw_report_info_tmp.fingers[i].wx	= F11_DATA_WX(fingerInfo);
		fw_report_info_tmp.fingers[i].wy	= F11_DATA_WY(fingerInfo);
		fw_report_info_tmp.fingers[i].z		= F11_DATA_Z(fingerInfo);

		if(has_8bitW != 0){
			fw_report_info_tmp.fingers[i].wx	|= ((buf[base + (fingerMax * 5) + 1 + i] << 0x04) & 0xF0);
			fw_report_info_tmp.fingers[i].wy	|=  (buf[base + (fingerMax * 5) + 1 + i] & 0xF0);
		}
#else
		fingerInfo = &buf[i * 8];
