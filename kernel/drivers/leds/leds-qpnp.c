...
static int qpnp_flash_set(struct qpnp_led_data *led)
{
	...
	if (val > 0) {
		if (led->flash_cfg->torch_enable) {
			#if defined(CONFIG_ARCH_MSM8226)	
			if (torch_cur_val == led->flash_cfg->current_prgm) {
				return 0;
			}
			#endif
#if 0
			#if defined(CONFIG_MACH_PA25) || defined(CONFIG_MACH_PB25) || defined(CONFIG_MACH_KF10)
			if(led->flash_cfg->current_prgm <= 1) {
			...
			rc = qpnp_led_masked_write(led,
				...
				led->flash_cfg->current_prgm);
			#if defined(CONFIG_ARCH_MSM8226)
			torch_cur_val = led->flash_cfg->current_prgm;
			#endif
			if (rc) {
				dev_err(&led->spmi_dev->dev,
			...
			//pr_err("qpnp_flash_set FLASH_NEED_DELAY_US=%d start_1\n", FLASH_NEED_DELAY_US);	
            usleep(FLASH_NEED_DELAY_US);
			//pr_err("qpnp_flash_set FLASH_NEED_DELAY_US=%d end_1\n", FLASH_NEED_DELAY_US);	
			...
			//pr_err("qpnp_flash_set FLASH_NEED_DELAY_US=%d start_2\n", FLASH_NEED_DELAY_US);	
            usleep(FLASH_NEED_DELAY_US);
			//pr_err("qpnp_flash_set FLASH_NEED_DELAY_US=%d end_2\n", FLASH_NEED_DELAY_US);	
...
static ssize_t flash_delay_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	ssize_t ret;
	u32 flash_delay_us=100;

	////pr_err("flash_delay_store flash_delay_us=%d\n", flash_delay_us);
	//pr_err("flash_delay_store old FLASH_NEED_DELAY_US=%d\n", FLASH_NEED_DELAY_US);	
	
	ret = kstrtou32(buf, 10, &flash_delay_us);
	if (ret)
		return ret;
	
    FLASH_NEED_DELAY_US=flash_delay_us;
	//pr_err("flash_delay_store new FLASH_NEED_DELAY_US=%d\n", FLASH_NEED_DELAY_US);	
	
	return count;
}

static DEVICE_ATTR(flash_delay, 0664, NULL, flash_delay_store);

static struct attribute *pwm_attrs[] = {
	&dev_attr_pwm_us.attr,
	&dev_attr_flash_delay.attr,
	NULL
};
