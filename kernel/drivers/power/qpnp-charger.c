...
#ifdef CONFIG_USE_SH_SWIC
	bool				vbdet_state;
	bool				vcdet_state;
	bool				vbus_state;
	struct timer_list		vbdet_timer;
	struct delayed_work		swic_check_work;
	struct wake_lock	swic_wake_lock;
#endif
};

static int cradledet_gpio = 0;

#ifdef CONFIG_USE_SH_SWIC
enum {
	SWITCH_STATE_USB,
	SWITCH_STATE_CRADLE,
	SWITCH_STATE_NO_CHANGE
};
static int vbdet_gpio = -1;
static int vcdet_gpio = -1;
static int swic_det_gpio = -1;

#define SWIC_CHECK_WORK_DELAY 500 //500ms
#define SWIC_WAKE_LOCK_DELAY 1000

#define VBDET_SECOND 1 //1m

static int qpnp_chg_switch_charger(int vbdet_state, int vcdet_state, int vbus_state);
static void vbdet_timer_fn(unsigned long data);
#endif
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	wake_lock_timeout(&chip->swic_wake_lock, msecs_to_jiffies(SWIC_WAKE_LOCK_DELAY));
	schedule_delayed_work(&chip->swic_check_work, msecs_to_jiffies(SWIC_CHECK_WORK_DELAY));

	pr_debug("%s exit\n", __func__);
#else /* CONFIG_USE_SH_SWIC */
	struct qpnp_chg_chip *chip = _chip;
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			if (cradledet_gpio > 0)
			{
				if (gpio_get_value(cradledet_gpio))
...
qpnp_charger_read_dt_props(struct qpnp_chg_chip *chip)
{
	...
	cradledet_gpio = of_get_named_gpio(chip->spmi->dev.of_node, "qcom,cradledet-gpio", 0);
	if (cradledet_gpio > 0)
	{
		rc = gpio_request(cradledet_gpio, "CRADLEDET-GPIO");
		if (rc) {
			pr_err("gpio_request failed %d %s \n", cradledet_gpio, "CRADLEDET-GPIO");
		}
		gpio_tlmm_config(GPIO_CFG(cradledet_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}

#ifdef CONFIG_USE_SH_SWIC
	vbdet_gpio = of_get_named_gpio(chip->spmi->dev.of_node, "qcom,vbdet-gpio", 0);
	vcdet_gpio = of_get_named_gpio(chip->spmi->dev.of_node, "qcom,vcdet-gpio", 0);
	swic_det_gpio = of_get_named_gpio(chip->spmi->dev.of_node, "qcom,swic-det-gpio", 0);

	if (vbdet_gpio <= 0 || vcdet_gpio <= 0 || swic_det_gpio <= 0) {
		pr_err("failed to get gpio, vbdet_gpio: %d vcdet_gpio: %d swic_det_gpio %d\n", vbdet_gpio, vcdet_gpio, swic_det_gpio);
		return -EINVAL;
	} else {
		rc = gpio_request(vbdet_gpio, "VBDET-GPIO");
		if (rc) {
			pr_err("gpio_request failed %d %s \n", vbdet_gpio, "VBDET-GPIO");
		}

		gpio_tlmm_config(GPIO_CFG(vbdet_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);

		rc = gpio_request(vcdet_gpio, "VCDET-GPIO");
		if (rc) {
			pr_err("gpio_request failed %d %s \n", vcdet_gpio, "VCDET-GPIO");

		}

		gpio_tlmm_config(GPIO_CFG(vcdet_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);

		rc = gpio_request(swic_det_gpio, "SWIC-DET-GPIO");
		if (rc) {
			pr_err("gpio_request failed %d %s \n", swic_det_gpio, "SWIC-DET_GPIO");
		}
		gpio_tlmm_config(GPIO_CFG(swic_det_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}
#endif
...
/*
 * switch charger
 * return:  SWITCH_STATE_USB usb charger switch
 *			SWITCH_STATE_CRADLE cradle charger switch
 *			SWITCH_STATE_NO_CHANGE
 */
static int qpnp_chg_switch_charger(int vbdet_state, int vcdet_state, int vbus_state)
{
	...
	pr_debug("current vbdet_state:%d next vbdet_state:%d\n", chip->vbdet_state, vbdet_state);
	pr_debug("current vcdet_state:%d next vcdet_state:%d\n", chip->vcdet_state, vcdet_state);
	pr_debug("current vbus_state:%d next vbus_state:%d\n", chip->vbus_state, vbus_state);

	if(vbus_state != chip->vbus_state) {
		chip->vbus_state = vbus_state;
	}

	if (vbdet_state != chip->vbdet_state) {
		if (vbus_state == 0 && vbdet_state == 1) {
			// vbdet 0 0 1 , handle abnormal
			pr_err("%s: vbdet_state abnormal, vbus_state = %d, vbdet_state = %d\n",
												__func__, vbus_state, vbdet_state);
		} else if (vbus_state == 1 && vbdet_state == 1) {
			// vbdet 0 1 1 , handle usb connection 
			pr_debug("%s: usb connection, vbus_state = %d, vbdet_state = %d\n",
												__func__, vbus_state, vbdet_state);
		} else if (vbdet_state == 0) {
			// vbdet 1 0 0 / 1 1 0, handle usb disconnection
			pr_debug("%s: usb disconnection, vbus_state = %d, vbdet_state = %d\n",
												__func__, vbus_state, vbdet_state);
		}

		chip->vbdet_state = vbdet_state;

		return SWITCH_STATE_USB;
	} else {
		if (vbus_state == 0 && vbdet_state == 1) {
			// vbdet 1 0 1 , handle abnormal
			pr_err("%s: vbdet_state abnormal, vbus_state = %d, vbdet_state = %d\n",
												__func__, vbus_state, vbdet_state);
		}
	}

	if (vcdet_state != chip->vcdet_state) {
		if (vbus_state == 0 && vcdet_state == 1) {
			//vcdet 0 0 1 , handle abnormal 
			pr_err("%s: vcdet_state abnormal, vbus_state = %d, vcdet_state = %d\n",
												__func__, vbus_state, vcdet_state);
		} else if (vbus_state == 1 && vcdet_state == 1) {
			// vcdet 0 1 1 , handle cradle connection
			// when cradle charger connect, start timer to check usb charger state every 1 second
			pr_debug("%s: cradle connection, vbus_state = %d, vcdet_state = %d\n",
												__func__, vbus_state, vcdet_state);
			qpnp_chg_force_run_on_batt(chip, 1);
			shchg_api_notify_cradle_connected();

			// start timer
			if(!timer_pending(&(chip->vbdet_timer))) {
				chip->vbdet_timer.function = vbdet_timer_fn;
				chip->vbdet_timer.data = 0;
				chip->vbdet_timer.expires = jiffies + VBDET_SECOND * HZ;
				add_timer(&(chip->vbdet_timer));
			}

		} else if (vcdet_state == 0) {
			// vcdet 1 0 0 / 1 1 0 , handle cradle disconnection
			pr_debug("%s: cradle disconnection, vbus_state = %d, vcdet_state = %d\n",
												__func__, vbus_state, vcdet_state);
			qpnp_chg_force_run_on_batt(chip, 0);
			shchg_api_notify_cradle_disconnected();
			...
			// del timer
			if(timer_pending(&(chip->vbdet_timer))) {
				del_timer(&chip->vbdet_timer);
			}
		}

		chip->vcdet_state = vcdet_state;

		return SWITCH_STATE_CRADLE;
	} else {
		if (vbus_state == 0 && vcdet_state == 1) {
			// vcdet 1 0 1 , handle abnormal
			pr_err("%s: vcdet_state abnormal, vbus_state = %d, vcdet_state = %d\n",
												__func__, vbus_state, vcdet_state);
		}
	}

	return SWITCH_STATE_NO_CHANGE;
}
...
static void vbdet_timer_fn(unsigned long data)
{
	...
	// handle when cradle connection, check usb states every 1s, in PA27 this is option, so no need to do
#if 0
	int vbdet_state = 0;
	int vcdet_state = 0;
	int vbus_state = 0;
	...
	if(vbdet_gpio > 0) {
		/*gpio is 'L' when usb charger is connect*/
		vbdet_state = gpio_get_value(vbdet_gpio) ? 0 : 1;
	}
	if(vcdet_gpio > 0) {
		/*gpio is 'L' when cradle charger is connect*/
		vcdet_state = gpio_get_value(vcdet_gpio) ? 0 : 1;
	}

	//TODO get vbus_state
	vbus_state = qpnp_chg_is_usb_chg_plugged_in(chip);

	if(chip->vbdet_state != vbdet_state) {
	   qpnp_chg_usb_usbin_valid_irq_handler(chip->usbin_valid.irq, chip);
	}
#endif

	mod_timer(&(chip->vbdet_timer), jiffies + VBDET_SECOND * HZ);
	pr_debug("%s exit\n", __func__);
}

static void qpnp_swic_check_work(struct work_struct *work)
{
	struct qpnp_chg_chip *chip = the_chip;
	int usb_present, host_mode, usbin_health;
	u8 psy_health_sts;
	...
	swic_det = gpio_get_value(swic_det_gpio);

	if(hw_type == 67 && swic_det == 1) {
		if(vbdet_gpio > 0) {
			/*gpio is 'L' when usb charger is connect*/
			vbdet_state = gpio_get_value(vbdet_gpio) ? 0 : 1;
		}
		...
		/* Judge usb charger or cradle charger 
		 * if usb charger, go forward process
		 * if cradle charger, call shchg_api_notify_cradle_connected() or shchg_api_notify_cradle_disconnected()
		 */
		switch_state = qpnp_chg_switch_charger(vbdet_state, vcdet_state, vbus_state);
		if(switch_state == SWITCH_STATE_CRADLE || switch_state == SWITCH_STATE_NO_CHANGE) {
			return;
		}

		usb_present = qpnp_chg_is_usb_chg_plugged_in(chip) && vbdet_state;
	} else {
		usb_present = qpnp_chg_is_usb_chg_plugged_in(chip);
	}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	init_timer(&(chip->vbdet_timer));
	INIT_DELAYED_WORK(&chip->swic_check_work, qpnp_swic_check_work);
	wake_lock_init(&chip->swic_wake_lock, WAKE_LOCK_SUSPEND, "swic_wake_lock");
#endif
	...
	if(hw_type == 67 && swic_det == 1) {
		...
		power_supply_set_present(chip->usb_psy, vbus_state && vbdet_state);

		/* Set USB psy online to avoid userspace from shutting down if battery
		 * capacity is at zero and no chargers online. */
		if (vbus_state && vbdet_state) {
			power_supply_set_online(chip->usb_psy, 1);
		}

		if (vbus_state && vcdet_state) {
			shchg_api_notify_cradle_connected();
		}
	} else {
		power_supply_set_present(chip->usb_psy, vbus_state);
...
#ifdef CONFIG_USE_SH_SWIC
	del_timer(&chip->vbdet_timer);
	cancel_delayed_work_sync(&chip->swic_check_work);
	wake_lock_destroy(&chip->swic_wake_lock);
#endif
